...
async function callClaudeApi(prompt: string) {
  try {
    ...
    // Check for API key - throw error if not configured
    if (!claudeApiKey) {
      console.error("Claude API key not configured");
      throw new Error("API_KEY_MISSING: Claude API key is not configured. Please set the ANTHROPIC_API_KEY secret in your Supabase project.");
    }
